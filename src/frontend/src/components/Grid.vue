<template>
  <div
    v-if="!fetchGridDataSuccessful"
    class="w-full flex flex-col text-center justify-center"
  >
    <p style="color: red">Unable to fetch grid data</p>
  </div>
  <div
    v-else-if="props.location === 'HCC2' && rowData && rowData.length > 0"
    class="ml-6 mr-2 mb-1"
  >
    <ag-grid-vue
      class="customGrid"
      :theme="myTheme"
...
const props = defineProps({
  ...
  selectedLocation: {
    // passed in from radio button ref
    type: String,
    required: true,
  },
  selectedShiftSize: {
...
async function fetchGridData(url = "") {
  //pass in optional url for compressed grid format
  try {
    ...
    if (error.response) {
      ...
      console.error(
        `Network error for ${props.day}, ${props.location}:`,
        error.message
      );
      console.error("Request:", error.request);
    } else {
      console.error(
        `Grid fetch error for ${props.day}, ${props.location}:`,
        error.message
      );
    }
...
async function handleCellClick(params) {
  ...
  try {
    const response = await axios.post(
      API_BASE_URL + endpoints.allocateShift,
      ALLOCATE_SHIFT_PAYLOAD
    );
    await fetchGridData();
...
async function addName(name) {
  try {
    ...
    const response = await axios.post(
      API_BASE_URL + endpoints.addName,
      ADD_NAME_PAYLOAD
    );
    await fetchGridData();
    toast.add({
...
async function removeName(name) {
  try {
    ...
    const response = await axios.delete(API_BASE_URL + endpoints.removeName, {
      data: REMOVE_NAME_PAYLOAD,
    });
    await fetchGridData();
    toast.add({
      severity: "info",
...
async function swapNames(names) {
  if (names.length !== 2) {
    console.log(
      "swapNames method expecting input array of containing 2 strings"
    );
    return;
  }
  ...
  try {
    const response = await axios.post(
      API_BASE_URL + endpoints.swapNames,
      SWAP_NAMES_PAYLOAD
    );
    // at check here, if compressed, we maintain compressed format
    await fetchGridData(isCompressed() ? API_BASE_URL + endpoints.compress : "");
    toast.add({
      severity: "info",
      summary: "Swap Name",
      detail: `swapped ${names}`,
      life: "4000",
    });
  } catch (error) {
    toast.add({
      severity: "error",
      summary: "Swap Name Failed",
      detail: "Internal Server Eror",
      life: "4000",
    });
    console.log(`Grid for ${props.day}, ${props.location}:`, error);
